...
int UVCPreview::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
    ...
    pthread_mutex_lock(&capture_mutex);
    {
        ...
        if (!env->IsSameObject(mFrameCallbackObj, frame_callback_obj)) {
            ...
            if (mFrameCallbackObj) {
                LOGW("DeleteGlobalRef :: mFrameCallbackObj");
                env->DeleteGlobalRef(mFrameCallbackObj);
            }
//            if (frame_callback_obj)
//            LOGW("1 mPixelFormat : %d", pixel_format);
            mFrameCallbackObj = env->NewGlobalRef(frame_callback_obj);
...
//                env->ExceptionClear();
                jclass clazz = env->FindClass("com/serenegiant/usb/IFrameCallback");
                if (LIKELY(clazz)) {
                    LOGW("success to get object class");
        ...
        if (frame_callback_obj) {
//            LOGW("2 mPixelFormat = %d", pixel_format);
            mPixelFormat = pixel_format;
            callbackPixelFormatChanged();
        }
    }
//    LOGW("3 mPixelFormat = %d", pixel_format);
...
//            LOGW("callbackPixelBytes = %d", callbackPixelBytes);
            jbyteArray buf = env->NewByteArray(callbackPixelBytes);
            env->SetByteArrayRegion(buf, 0, callbackPixelBytes,
                                    reinterpret_cast<jbyte *>(callback_frame->data));
            env->ExceptionClear();
            if (iframecallback_fields.onFrame) {
                //送数据给Java的onFrame函数
